...
class SufficientVelocity(Source):
    ...
    def novel(self, url: str) -> Novel:
        ...
        for i, a in enumerate(
            soup.select(
                ".block-body--threadmarkBody.is-active > div > div > div:first-child a"
            )
        ):
            chapter = Chapter(
                index=i, title=a.text, url=self.to_absolute_url(a["href"])
            )
    ...
    def chapter(self, chapter: Chapter):
        ...
        paragraphs = [
            t for t in self.clear_tags.sub("", str(content)).splitlines() if t
        ]

        chapter.title = article.select_one(
            ".message-cell--threadmark-header > span"
        ).text.strip()
